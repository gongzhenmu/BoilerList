...
router.get('/FindPostByPostID/:postID', (req, res)=>{
  Post.find({ postID: req.params.postID }, function(err, data) {
    if (err) {
      console.log('Find post by postID failed.', err);
      return res.status(400).send(err);
    } else {
      console.log('Find post by postID success!');
      return res.status(200).send(data);
    }
  })
});

router.get('/FindPostByTitle/:title', (req, res)=>{
  Post.find({ title: req.params.title },  function(err, data) {
    if (err) {
      console.log('Find post by title failed.', err);
      return res.status(400).send(err);
    } else {
      console.log('Find post by title success!');
      return res.status(200).send(data);
    }
  })
});
